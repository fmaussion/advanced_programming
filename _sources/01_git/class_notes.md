...
7. If we want to update the local repository to reflect the changes in the Holy repo (example with the ACINN repo): `git remote add upstream git@git.uibk.ac.at/acinn/icu/advpro_ss23`. "Upstream" here is the address of the Holy repo, from which we will be able to fetch the changes, whereas "origin" is still our fork. To check all our remotes, do `git remote -v`.
